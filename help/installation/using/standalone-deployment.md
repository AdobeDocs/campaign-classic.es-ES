...
1. Una vez instalado el servidor Adobe Campaign, inicio el servidor de aplicaciones (web) mediante el comando **nlserver web -tomcat** (el módulo Web le permite inicio de Tomcat en modo de servidor Web independiente escuchando en el puerto 8080) y para asegurarse de que Tomcat inicio correctamente:
...
1. Cree la instancia de **demostración** con las máscaras DNS para el seguimiento (en este caso, **tracking.campaña.net**) y el acceso a las consolas de cliente (en este caso, **console.campaña.net**). Hay dos formas de hacerlo:
...
1. Edite el archivo **config-demo.xml** (creado en el paso anterior junto a **config-default.xml**) y asegúrese de que los procesos **mta** (envío), **wfserver** (flujo de trabajo), **inMail** **** (correos devueltos) y stat (estadísticas) están habilitados. A continuación, configure la dirección del servidor de estadísticas:
...
1. Inicio la consola de cliente de Adobe Campaign (desde la página de descarga anterior o se inicia directamente en el servidor para una instalación de Windows), establezca la URL de conexión del servidor en https://console.campaign.net y conéctese mediante el inicio de sesión **interno** .
...
1. Vuelva a iniciar sesión en la consola del cliente con el inicio de sesión del **administrador** sin contraseña y inicio el asistente de implementación ( **[!UICONTROL Tools > Advanced]** menú ) para finalizar la configuración de la instancia.
