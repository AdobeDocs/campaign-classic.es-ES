...
1. Una secuencia de comandos **VBS** permite configurar automáticamente los recursos utilizados por el servidor de Adobe Campaign en el directorio virtual que acabamos de crear. Para iniciarlo, haga doble clic en el archivo **is_neolane_setup.vbs** ubicado en la `[INSTALL]\tomcat-7\conf` carpeta, donde `[INSTALL]` es la ruta para acceder a la carpeta de instalación de Adobe Campaign.
...
1. En la **[!UICONTROL Content View]** ficha, asegúrese de que el sitio web está correctamente configurado con los recursos de Adobe Campaign:
...
Por ejemplo, si utiliza una actividad **de carga de datos (archivo)** en un flujo de trabajo para importar un archivo de 50 MB, un error impedirá que el flujo de trabajo se ejecute correctamente.

En este caso, debe aumentar este límite:

1. Abra IIS a través del **[!UICONTROL Start > (Control panel) > Administration tools]** menú.
1. En el panel **Conexiones** , seleccione el sitio creado para la instalación de Adobe y, a continuación, haga doble clic en Filtrado de **solicitudes** en el panel principal.
...
1. En el panel **Conexiones** , seleccione el sitio creado para la instalación de Adobe Campaign y, a continuación, haga doble clic en el editor **** de configuración en el panel principal.
