...
Algunas partes de esta configuración son un desarrollo personalizado y requieren lo siguiente:
...
Dado que la edición del código JS requiere habilidades técnicas, no lo intente sin la comprensión adecuada.
...
Debe reiniciar [!DNL pipelined] después de editar el JS.

### Activar formato de datos {#trigger-format}

Los [!DNL trigger] datos se pasan a la función JS en formato XML.
...
### Enriquecimiento de formato de datos {#enrichment-format}
...
* **[!UICONTROL eVar01]** puede contener la ID del comprador en formato de cadena que se utiliza para reconciliar con destinatarios de Adobe Campaign. <br>Debe conciliarse para encontrar el ID del comprador, que es la clave primaria.

* **[!UICONTROL timeGMT]** puede contener la hora del activador en el lado de Adobe Analytics en formato UTC Epoch (segundos desde 01/01/1970 UTC).
...
Para fines de depuración y supervisión, los datos de activación completos se escriben en la tabla de activación del campo &quot;data&quot; en formato XML. De forma alternativa, el logInfo() que contenga los datos desencadenadores tiene el mismo propósito.
...
Los Eventos se almacenan en una tabla de la base de datos. Se emite mediante campañas de marketing a clientes de destinatario y enriquece los correos electrónicos mediante activadores.
...
Puede resultar difícil de implementar si no se establece ningún índice en shopper_id. Si los criterios se encuentran en un servidor de base de datos independiente al servidor de marketing, utiliza un vínculo de base de datos, que tiene un rendimiento deficiente.
