...
Para comprender mejor las tablas integradas de Campaign y su interacción, consulte [esta sección](../../configuration/using/about-data-model.md) para obtener más información.

Leer [esta documentación](../../configuration/using/about-schema-reference.md) para empezar a utilizar los esquemas de Campaign. Obtenga información sobre cómo configurar esquemas de extensión para ampliar el modelo de datos conceptuales de la base de datos de Adobe Campaign en [este documento](../../configuration/using/about-schema-edition.md).
...
Este enfoque centrado en el cliente se muestra en el gráfico siguiente. La variable **Destinatario** representa la tabla de cliente principal en torno a la cual se está creando todo:
...
El modelo de datos predeterminado de Adobe Campaign se muestra en [este documento](../../configuration/using/data-model-description.md).

>[!NOTE]
>
>Adobe Campaign Classic permite crear una tabla de cliente personalizada. Sin embargo, en la mayoría de los casos, se recomienda aprovechar el [Tabla de destinatarios](../../configuration/using/about-data-model.md#default-recipient-table) que ya tiene tablas y funciones adicionales prediseñadas.
...
* La longitud de un **string** siempre se debe definir con la columna . De forma predeterminada, la longitud máxima en Adobe Campaign es 255, pero Adobe recomienda mantener el campo más corto si ya sabe que el tamaño no excederá una longitud más corta.
...
Este valor se toma de lo que se denomina una **Secuencia**, que es un objeto de base de datos que se utiliza para generar una secuencia numérica.
...
1. Haga clic con el botón derecho en el **[!UICONTROL First name]** campo .
...
Ahora puede realizar el filtrado de búsqueda en el **[!UICONTROL First name]** y **[!UICONTROL Last name]** campos según las distintas condiciones de filtro.
...
Los vínculos creados por y modificados por última vez están presentes en muchas tablas. Es posible deshabilitar el índice utilizando el atributo noDbIndex en el vínculo, si la empresa no está utilizando esta información.
...
Descubra más información sobre el flujo de trabajo de limpieza de la base de datos de Campaign [en esta sección](../../production/using/database-cleanup-workflow.md).
...
En PostgreSQL, una fila no debe superar los 8 KB para evitar [TOAST](https://wiki.postgresql.org/wiki/TOAST) mecanismo. Por lo tanto, intente reducir en la medida de lo posible el número de columnas y el tamaño de cada fila para preservar el rendimiento óptimo del sistema (memoria y CPU).
...
En este ejemplo:
* La variable *Transacción* y *Artículo de transacción* las tablas son grandes: más de 10 millones.
