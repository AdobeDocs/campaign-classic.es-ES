---
title: Flujo de trabajo de limpieza de bases de datos
seo-title: Flujo de trabajo de limpieza de bases de datos
description: Flujo de trabajo de limpieza de bases de datos
...
# Flujo de trabajo de limpieza de bases de datos{#database-cleanup-workflow}
...
La limpieza de la base de datos está configurada en dos niveles: en el programador de flujo de trabajo y en el asistente de implementación.

### El programador {#the-scheduler}

>[!NOTE]
>
>For more on the scheduler, refer to [this section](../../workflow/using/scheduler.md).

De forma predeterminada, el flujo de trabajo se configura para que se inicie diariamente a las 4 de la madrugada. **[!UICONTROL Database cleanup]** El programador le permite cambiar la frecuencia de activación del flujo de trabajo. Están disponibles las siguientes frecuencias:
...
>[!CAUTION]
>
>Para que el flujo de trabajo **[!UICONTROL Database cleanup]** se inicie en la fecha y hora definidas en el programador, se debe iniciar el motor de flujo de trabajo (wfserver). Si este no es el caso, la limpieza de la base de datos no tendrá lugar hasta la próxima vez que se inicie el motor de flujo de trabajo.
...
* Seguimiento consolidado: **NmsCleanup_TrackingStatPurgeDelay** (consulte [Limpieza de registros](#cleanup-of-tracking-logs)de seguimiento)
* Registros de envío: **NmsCleanup_BroadLogPurgeDelay** (consulte [Limpieza de registros](#cleanup-of-delivery-logs)de entrega)
* Registros de seguimiento: **NmsCleanup_TrackingLogPurgeDelay** (consulte [Limpieza de registros](#cleanup-of-tracking-logs)de seguimiento)
* Entregas eliminadas: **NmsCleanup_RecycledDeliveryPurgeDelay** (consulte [Limpieza de envíos que se eliminarán o reciclarán](#cleanup-of-deliveries-to-be-deleted-or-recycled))
...
* Perfiles del visitante: **NmsCleanup_VisitorPurgeDelay** (consulte [Limpieza de visitantes](#cleanup-of-visitors))
...
* Eventos: **NmsCleanup_EventPurgeDelay** (consulte [Limpieza de eventos](#cleansing-expired-events)caducados)
* Eventos archivados: **NmsCleanup_EventHistoPurgeDelay** (consulte [Limpieza de eventos](#cleansing-expired-events)caducados)
...
En la fecha y hora definidas en el programador de flujo de trabajo (consulte [El programador](#the-scheduler)), el motor de flujo de trabajo inicia el proceso de limpieza de la base de datos. La limpieza de base de datos se conecta a la base de datos y ejecuta las tareas en la secuencia que se muestra a continuación.

>[!CAUTION]
>
>Si se produce un error en una de estas tareas, no se ejecutarán las siguientes.\
...
1. Cuando la lista es una lista de tipos &#39;Lista&#39;, la tabla asociada se elimina mediante la siguiente consulta:

   ```
   DROP TABLE grp$(l)
   ```

1. Cada lista **Seleccionar** tipo recuperada por la operación se elimina mediante la siguiente consulta:
...
### Limpieza de las entregas que deben eliminarse o reciclarse {#cleanup-of-deliveries-to-be-deleted-or-recycled}

Esta tarea purga todos los envíos que se van a eliminar o reciclar.
...
1. Para cada servidor de fuentes intermedias, la tarea selecciona la lista de entregas que se van a eliminar.
1. El flujo de trabajo elimina los registros de entrega, los archivos adjuntos, la información de la página de reflejo y todos los demás datos relacionados. **[!UICONTROL Database cleanup]**
1. Antes de eliminar definitivamente la entrega, el flujo de trabajo purga la información vinculada de las siguientes tablas:
...
#### Entregas que utilizan fuentes intermedias {#deliveries-using-mid-sourcing}

El flujo de trabajo también elimina los envíos en los servidores de abastecimiento medio. **[!UICONTROL Database cleanup]**

1. Para ello, el flujo de trabajo comprueba que cada entrega está inactiva (según su estado). Si un envío está activo, se detendrá antes de eliminarse. La comprobación se realiza mediante la ejecución de la siguiente consulta:
...
### Limpieza de entregas vencidas {#cleanup-of-expired-deliveries}

Esta tarea detiene las entregas cuyo período de validez ha caducado.

1. El flujo de trabajo crea la lista de entregas que han caducado. **[!UICONTROL Database cleanup]** Esta lista incluye todos los envíos caducados con un estado distinto a **[!UICONTROL Finished]** , así como los envíos detenidos recientemente con más de 10.000 mensajes no procesados. Se utiliza la siguiente consulta:

   ```
   SELECT iDeliveryId, iState FROM NmsDelivery WHERE iDeleteStatus=0 AND iIsModel=0 AND iDeliveryMode=1 AND ( (iState >= 51 AND iState < 85 AND tsValidity IS NOT NULL AND tsValidity < $(currentDate) ) OR (iState = 85 AND DateMinusDays(15) < tsLastModified AND iToDeliver - iProcessed >= 10000 ))
   ```

   donde el modo **de entrega 1** coincide con el **[!UICONTROL Mass delivery]** modo, el **estado 51** coincide con el **[!UICONTROL Start pending]** estado, el **estado 85** coincide con el **[!UICONTROL Stopped]** estado y el mayor número de registros de entrega actualizados masivamente en el servidor de entrega es igual a 10.000.

1. A continuación, el flujo de trabajo incluye la lista de entregas caducadas recientemente que utilizan el abastecimiento intermedio. Se excluyen las entregas para las que aún no se han recuperado registros de entrega a través del servidor de abastecimiento intermedio.
...
1. En la lista de entregas caducadas, los registros de entrega cuyo estado es **[!UICONTROL Pending]** , cambie a **[!UICONTROL Delivery cancelled]** y todas las entregas de esta lista cambiarán a **[!UICONTROL Finished]** .
   ...
   donde **$(curdate)** es la fecha actual del servidor de bases de datos, **$(bl)** es el identificador del mensaje de registros de entrega, **$(dl)** es el identificador de entrega, el estado de **entrega 6** coincide con el estado **[!UICONTROL Pending]** y el estado de **** **[!UICONTROL Delivery cancelled]** entrega 7 coincide con el estado de la solicitud.

   ```
   UPDATE NmsDelivery SET iState = 95, tsLastModified = $(curdate), tsBroadEnd = tsValidity WHERE iDeliveryId = $(dl)
   ```

   donde el estado de **entrega 95** coincide con el **[!UICONTROL Finished]** estado y **$(dl)** es el identificador de la entrega.
...
1. Todos los fragmentos (**deliveryParts**) de entregas obsoletas se eliminan y todos los fragmentos obsoletos de entregas de notificaciones en curso se eliminan. La eliminación masiva se utiliza para ambas tareas.
   ...
   donde el estado de **entrega 95** coincide con el **[!UICONTROL Finished]** estado, el estado de **entrega 85** coincide con el **[!UICONTROL Stopped]** estado y **$(curDate)** con la fecha del servidor actual.

### Limpieza de las páginas reflejadas {#cleanup-of-mirror-pages}

Esta tarea elimina los recursos web (páginas de reflejo) que utilizan los envíos.

1. En primer lugar, la lista de entregas que se van a purgar se recupera mediante la siguiente consulta:
...
1. A continuación, se purga la tabla **NmsMirrorPageInfo** , si es necesario mediante el identificador de la entrega recuperada anteriormente. La eliminación masiva se utiliza para generar las siguientes consultas:
...
1. Luego se identifican las entregas purgadas para evitar tener que volver a procesarlas más tarde. Se ejecuta la siguiente consulta:

   ```
   UPDATE NmsDelivery SET iWebResPurged = 1 WHERE iDeliveryId IN ($(strIn))
   ```

   donde **$(strIn)** es la lista de identificadores de entrega.
...
1. A continuación, se excluyen las tablas utilizadas por los flujos de trabajo en curso. Para ello, la lista de entregas en curso se recupera mediante la siguiente consulta:

   ```
   SELECT iDeliveryId FROM NmsDelivery WHERE iDeliveryId<>0 AND iDeleteStatus=0 AND iState NOT IN (0,85,100);
   ```

   donde 0 es el valor que coincide con el estado de entrega, 85 coincide con el **[!UICONTROL Being edited]** estado y 100 con el **[!UICONTROL Stopped]** **[!UICONTROL Deleted]** estado.

1. Las tablas que ya no se utilizan se eliminarán mediante la siguiente consulta:
...
1. Todos los rechazos huérfanos se eliminan mediante la siguiente consulta:
...
Esta tarea purga cada instancia de flujo de trabajo mediante su identificador (**lWorkflowId**) y su historial (**lHistory**). Elimina las tablas inactivas ejecutando de nuevo la tarea de limpieza de la tabla de trabajo.
...
1. La eliminación masiva se utiliza para purgar todas las tablas de la lista de tablas previamente recuperadas. Se utiliza la siguiente consulta:
...
### Limpieza de los registros de entrega {#cleanup-of-delivery-logs}

Esta tarea le permite purgar los registros de entrega almacenados en varias tablas.

1. Para ello, la lista de esquemas de registro de entrega se recupera mediante la siguiente consulta:

   ```
   SELECT distinct(sBroadLogSchema) FROM NmsDeliveryMapping WHERE sBroadLogSchema IS NOT NULL UNION SELECT distinct(sBroadLogExclSchema) FROM NmsDeliveryMapping WHERE sBroadLogExclSchema IS NOT NULL
   ```

1. Al utilizar el abastecimiento intermedio, no se hace referencia a la tabla **NmsBroadLogMid** en las asignaciones de entrega. El esquema **nms:wideLogMid** se agrega a la lista recuperada por la consulta anterior.
1. A continuación, el flujo de trabajo de limpieza **de** bases de datos purga datos obsoletos de tablas recuperadas anteriormente. Se utiliza la siguiente consulta:
   ...
   donde **$(tableName)** es el nombre de cada tabla de la lista de esquemas, y **$(option)** es la fecha definida para la opción **NmsCleanup_BroadLogPurgeDelay** (consulte el Asistente [](#deployment-wizard)de implementación).
...
* **Fecha** inicial: fecha del siguiente proceso que coincide con la opción **NmsLastErrorStatCoalesce** o con la fecha más reciente de la tabla.
* **Fecha** final: fecha actual del servidor.

Si la fecha de inicio es mayor o igual que la fecha de finalización, no se realizará ningún proceso. En este caso, aparece el **mensaje coalesceUpToDate** .
...
donde **$end** y **$start** son las fechas de inicio y finalización definidas previamente.
...
La lista de tablas de proposiciones se recupera y la eliminación masiva se lleva a cabo en cada una de ellas, utilizando la siguiente consulta:
...
Esta tarea limpia las tablas de simulación huérfanas (que ya no están vinculadas a una simulación de oferta o a una simulación de entrega).
...
Si la opción **XtkCleanup_NoStats** no existe o si su valor es 0, se ejecutará la optimización del almacenamiento en modo detallado (VACUUM VERBOSE ANALYZE) en PostgreSQL y se actualizarán las estadísticas en todas las demás bases de datos. Para asegurarse de que este comando se ejecuta, compruebe los registros PostgreSQL. VACUUM generará líneas en formato: `INFO: vacuuming "public.nmsactivecontact"` y ANALYZE generará líneas en formato: `INFO: analyzing "public.nmsactivecontact"`.
...
Si el valor de la opción es 2, se ejecutará el análisis de almacenamiento en modo detallado (ANALYZE VERBOSE) en PostgreSQL y se actualizarán las estadísticas de todas las demás bases de datos. Para asegurarse de que este comando se ejecuta, compruebe los registros PostgreSQL. ANALYZE generará líneas en el formato: `INFO: analyzing "public.nmsactivecontact"`.
