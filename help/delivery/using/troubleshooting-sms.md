...
Consulte la sección [Información en cada tipo de PDU](../../delivery/using/sms-protocol.md#information-pdu) anterior para saber qué campo está disponible en `BIND PDU`, generalmente agrega el código corto en `address_range`, pero eso requiere soporte especial del proveedor. Póngase en contacto con ellos para saber cómo esperan enrutar varios códigos cortos de forma independiente.
...
## Problema con intermediaria (alojado){#issue-mid-sourcing}

* Si el problema se produce en un entorno de intermediaria, asegúrese de que el envío y los registros generales se hayan creado y actualizado correctamente en el servidor intermediaria. Si ese no es el caso, este no es un problema de SMS.

* Si todo funciona en el servidor central y SMS se envían correctamente, pero la instancia de marketing no se actualiza correctamente, es posible que tenga un problema de sincronización intermedia.
...
* Si el proveedor cierra las conexiones enviando un `TCP FIN` o un `TCP RST packet`, pregunte a su proveedor más información.
...
* Si reiniciar el MTA hace que el envío de MT funcione de nuevo durante un período de tiempo pequeño, probablemente tenga problemas debido a una conexión inestable. Consulte la sección [Problema con conexiones inestables](../../delivery/using/troubleshooting-sms.md#issues-unstable-connection).
...
* Si ve muchos `BIND/UNBIND`, tiene una conexión inestable. Consulte la sección[Problema con conexiones inestables](../../delivery/using/troubleshooting-sms.md#issues-unstable-connection) para obtener soluciones antes de intentar resolver problemas de mensajes de duplicado.
...
* Compruebe que Adobe Campaign responde con éxito `DELIVER_SM_RESP PDU` de manera oportuna. En Adobe Campaign Classic, esto garantiza que el SR se haya insertado en la tabla `providerMsgId` para el procesamiento diferido por el proceso SMS.
...
Si el `DELIVER_SM PDU` ha sido reconocido por el conector SMPP extendido de Adobe Campaign Classic pero el archivo wideLog no se actualiza correctamente, compruebe el proceso de reconciliación de ID descrito en la sección [Entradas de MT, SR y de registro de banda ancha](../../delivery/using/sms-protocol.md#matching-mt).
...
* Si aparece `DELIVER_SM PDU`, compruebe que Adobe Campaign lo ha reconocido con un `DELIVER_SM_RESP PDU` correcto (código 0). Este RESP garantiza que Adobe Campaign ha aplicado toda la lógica de procesamiento (respuesta automática y permitir/lista de bloqueados). Si no es el caso, busque un mensaje de error en los registros de proceso SMS.

* Si las respuestas automáticas están habilitadas, compruebe que `SUBMIT_SM` se haya enviado al proveedor. Si no es así, se garantiza que encontrará un mensaje de error en los registros de proceso de SMS.
...
1. En la **Cuenta externa**, marque **Habilitar los rastros SMPP detallados en el archivo de registro**.
