...
* El texto, que está limitado a 140 bytes, significa entre 70 y 160 caracteres en función de la codificación. Consulte [Codificación de texto SMS](../../delivery/using/sms-protocol.md#sms-text-encoding) a continuación para obtener detalles y limitaciones.
...
Adobe Campaign Classic admite la versión 3.4 del protocolo SMPP. Este es un protocolo muy extendido que permite enviar SMS a un proveedor (SMSC) y recibir SMS, así como recibos. Para obtener más información sobre esto, consulte la [documentación de SMPP](https://smpp.org/SMPP_v3_4_Issue1_2.pdf).
...
El sistema debe enviar esta PDU antes de desconectarla. Debe esperar la PDU `UNBIND_RESP` correspondiente antes de cerrar la conexión.
...
* **short_message**: del mensaje. Para SR, contiene datos descritos en el apéndice B de la especificación del protocolo SMPP. Consulte [Administración de errores de SR](../../delivery/using/sms-protocol.md#sr-error-management) para obtener más detalles.
...
Consulte la descripción de los campos `esm_class`, `short_message` y `message_payload` de la [PDU SUBMIT_SM](../../delivery/using/sms-protocol.md#information-pdu) para obtener más detalles sobre el protocolo y los formatos.
...
Para alcanzar el máximo rendimiento posible, tendrá que ajustar la ventana de envío máxima. La ventana de envío es el número de `SUBMIT_SM PDU`s que se puede enviar sin esperar a `SUBMIT_SM_RESP`. Consulte la sección [Configuración de la ventana de envío](../../delivery/using/sms-protocol.md#throughput-timeouts) para obtener más detalles.
...
Los conectores de Adobe Campaign Classic (excepto Extended SMPP) utilizan un comportamiento codificado que depende del proveedor seleccionado. El SMPP genérico sólo distingue entre éxito y error, sin ningún detalle. El registro de envíos puede contener información que no está garantizada.
...
### Ejemplo de procesamiento SR en Adobe Campaign Classic Extended SMPP genérico {#sr-processing}
...
Siempre debe **ponerse en contacto con el proveedor SMSC en caso de problemas de codificación**. Sólo los proveedores de SMSC tienen conocimiento de la codificación que admiten y de las reglas especiales que pueden aplicarse debido a limitaciones en su plataforma técnica.
...
Define el nombre de la implementación de SMSC. Debe configurarse en el nombre de su proveedor. Póngase en contacto con el administrador o con el equipo de entregabilidad para saber qué agregar en este campo. La función de este campo se describe en la sección [SR error management](../../delivery/using/sms-protocol.md#sr-error-management).
...
El conector SMPP ampliado de Adobe Campaign Classic puede controlar el número de conexiones por elemento secundario de MTA. Para controlar el límite global de conexiones, tendrá que limitar el número de procesos secundarios de MTA, lo que a menudo significa tener una plataforma de intermediaria dedicada para SMS.
...
Cuando se selecciona, se enviará SMS largos en una PDU SUBMIT_SM, colocando el texto en el campo opcional message_payload. Consulte la [especificación SMPP](../../delivery/using/sms-protocol.md#ACS-SMPP-connector) para obtener más información sobre esto.
...
>[!NOTE]
>
>Solo disponible en el conector SMPP ampliado de Adobe Campaign Classic.
...
>[!IMPORTANT]
>
>En Adobe Campaign Classic y en una arquitectura híbrida, la aplicación de la respuesta automática para el conector SMPP ampliado requiere agregar acceso de escritura para el operador medio en la carpeta **Cuenta externa**.
...
Al enviar piezas de envío, el MTA genera elementos secundarios de MTA. El número de procesos secundarios de MTA es dinámico y depende de una configuración en serverConf.xml. Cada elemento secundario de MTA crea una instancia del conector `CSmppConnectorWorker` que se conecta al proveedor de SMPP. Las conexiones se mantienen vivas mientras el elemento secundario de MTA se mantenga vivo, también se puede configurar en serverConf.xml.

El proceso SMS sólo procesa SR, se conecta al proveedor y deja la conexión abierta. El proceso se vuelve a conectar cada 10 minutos para volver a cargar la nueva configuración, lo cual es normal.

### Entradas MT, SR y de registro de banda ancha {#matching-mt} que coinciden

Se utiliza una tabla intermedia `nmsProviderMsgId` para almacenar los datos MT y SR temporalmente antes de enviarse asincrónicamente al diario.

`nmsProviderMsgId` tiene 3 grupos de columnas:

* Columnas actualizadas cuando se envía y se acepta un mensaje MT: `iBroadLogId`, `iDeliveryId`

* Columnas actualizadas cuando se recibe un SR: `iMsgId`, `iStatus`

* Columnas que siempre se actualizan: `tsCreated`, `sProviderId`

Cuando haya terminado el procesamiento de MT y SR, debe tener líneas completas, con información de registro de banda ancha e información de estado.
...
* El proceso SMS calcula el nombre de la tabla de diarios extendidos en función de la asignación de envíos.
* El proceso de SMS actualiza la tabla de diarios extendidos con el ID del mensaje y el estado.

**Conexión en paralelo y de rendimiento**

Cada elemento secundario de MTA crea una cantidad configurable de conexiones, por lo que limitar el número de elementos secundarios de MTA limitará el número de conexiones. Debido a que la correlación entre los procesos secundarios de MTA y el tráfico está correlacionada, esto puede estar controlado de alguna manera pero aún es un poco impredecible.
...
Compruebe que no hay demasiados `BIND_* PDU`s. Si hay demasiados, podría indicar que la conexión es inestable. Consulte la sección [Problemas con conexiones inestables](../../delivery/using/sms-protocol.md#issues-unstable-connection) para obtener más información.
